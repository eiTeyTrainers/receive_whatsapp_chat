...
import 'package:koala/src/column.dart';
...
import 'package:koala/src/utils/list_base.dart';
...
class DataFrame extends ListBase<RecordRow> {
 ...
 Map _group(String colName,
      {required bool inPlace,
      required bool ascending,
      required bool nullsFirst,
      required Comparator<Record>? compareRecords}) {
    final index = columnIndex(colName);
     var data  = inPlace ? this : ListListExtensions(this).copy();
    //  var map = {};
    //    for (var element in data) {
    //     (map[(Map obj) => obj['contact'](element)] ??= []).add(element);
    //   }
    var groups = {};
    var groupColumn = data.map<String?>((row) => row[index].toString()).toSet().toList(growable: false);
    for(var nameMap in groupColumn){
        String? name = nameMap;
       groups[name] = [];
    }
    for(var row in data){
      if(row != Null){
      var list= groups[row[index]];
      list.add(row);

      }
    
    }
        // groups[index] = [...].add(row[3]) ;
    
       return groups;
  }

  static int _groupCompareRecords(Record a, Record b, bool ascending,
      bool nullsFirst, Comparator<Record>? compare) {
    // return compare result if function given
    if (compare != null) return compare(a, b);
